...
`core/` and `cli/` are implemented and tested. `ui_tui/`, `ui_linux/`, `ui_mac/` and `ui_windows/`
do not exist yet. `input.txt` is the original design spec.
...
```sh
cargo test --workspace          # 31 tests: core unit tests + CLI end-to-end tests
cargo test -p editor-core       # core only
...
CI runs all of the above (`.github/workflows/core-cli.yml`). `rustfmt` and `clippy` are not
installed in every dev environment; CI is the enforcement point.
...
ui_tui/       ⬜ crossterm + ratatui terminal UI (pure Rust)
...
the CLI: tests on all three OS runners (the CLI ships everywhere), plus a single fmt/clippy job.
...
`ui_tui/` uses `crossterm` for raw terminal control and input, `ratatui` for layout and rendering —
not raw ncurses. The loop enables raw mode, switches to the alternate screen buffer, renders the
core's viewport into a `Paragraph`, positions the hardware cursor from the core's logical cursor
position, then blocks on `crossterm::event::read()`. Teardown must disable raw mode and leave the
alternate screen on **every** exit path, including panics — a missed teardown leaves the user's
terminal unusable.
